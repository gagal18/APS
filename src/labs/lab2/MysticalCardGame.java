...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> firstSorcererHead = firstSorcererCards.getFirst();
        Card mostSignificant = firstSorcererHead.element;
        while(firstSorcererHead != null){
            if(mostSignificant.significance() < firstSorcererHead.element.significance()){
                mostSignificant = firstSorcererHead.element;
            }
            firstSorcererHead = firstSorcererHead.succ;
        }
        firstSorcererCards.delete(firstSorcererCards.find(mostSignificant));
        int mid = secondSorcererCards.size() / 2 - 1;
        SLLNode<Card> midSecondCards = secondSorcererCards.getFirst();
        while(mid != 0){
            midSecondCards = midSecondCards.succ;
            mid--;
        }
        secondSorcererCards.insertAfter(mostSignificant , midSecondCards);
    }
