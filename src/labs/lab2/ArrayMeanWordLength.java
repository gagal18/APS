...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for (int i = 0; i < arr.getSize(); i++) {
            word_length += arr.get(i).length();
        }
        double avg_word_length = (double)word_length / arr.getSize();
        String word = arr.get(0);
        int diff = (int) Math.abs(arr.get(0).length() - avg_word_length);
        for (int i = 0; i < arr.getSize(); i++) {
            String toCheck = arr.get(i);
            int diffToCheck = (int) Math.abs(toCheck.length() - avg_word_length);
            if (toCheck.length() == avg_word_length) {
                return toCheck;
            }
            if (diffToCheck < diff || (diffToCheck == diff && toCheck.length() > word.length())) {
                word = toCheck;
                diff = diffToCheck;
            }
        }
